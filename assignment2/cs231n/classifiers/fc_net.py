...
import collections # zy
import cs231n.layers as layers # zy
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        out_affine1, cache_affine1 = layers.affine_forward(X,
                                                           self.params["W1"],
                                                           self.params["b1"])
        out_relu1, cache_relu1 = layers.relu_forward(out_affine1)
        out_affine2, cache_affine2 = layers.affine_forward(out_relu1,
                                                           self.params["W2"],
                                                           self.params["b2"])
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers-1):
            ...
            self.params["W"+n] = np.random.normal(scale=weight_scale,
                                                  size=(dim_in, dim_out))
            self.params["b"+n] = np.zeros(dim_out)

            if self.use_batchnorm:
                # (zy) we need a scale and shift for every input dimension
                # different for layer normalization, we only need one shift and
                # scale for every input (though different input need different
                # set of parameters), no matter how large its dimensions are.
                self.params["gamma"+n] = np.ones(dim_out)
                self.params["beta"+n] = np.zeros(dim_out)
            dim_in = dim_out

        nn = str(self.num_layers)
        self.params["W"+nn] = np.random.normal(scale=weight_scale,
                                               size=(hidden_dims[-1], num_classes))
        self.params["b"+nn] = np.zeros(num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.use_batchnorm:
                out_layer, cache = affine_bn_relu_forward(out_layer,
                                                          self.params["W"+n],
                                                          self.params["b"+n],
                                                          self.params["gamma"+n],
                                                          self.params["beta"+n],
                ...
                out_layer, cache = layers.affine_forward(out_layer,
                                                         self.params["W"+n],
                                                         self.params["b"+n])
            ...
            if self.use_dropout:
                out_layer, cache = layers.dropout_forward(out_layer,
                                                          self.dropout_param)
                caches["drop"].append(cache)

        nn = str(self.num_layers)
        scores, cache = layers.affine_forward(out_layer,
                                              self.params["W"+nn],
                                              self.params["b"+nn])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        for i in range(self.num_layers-2, -1, -1):
            ...
            if self.use_batchnorm:
                out = affine_bn_relu_backward(dout, caches["affine_bn_relu"][i])
                dout, grads["W"+n], grads["b"+n], \
                    grads["gamma"+n], grads["beta"+n] = out
                grads["W"+n] += self.reg*self.params["W"+n] if self.reg else 0
