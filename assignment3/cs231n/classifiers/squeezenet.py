...
def fire_module(x,inp,sp,e11p,e33p):
    ...
    # sp = output channel dimension for squeeze scope
    # e11p & e33p = output channel dimension for parallel paths.
    # e11p & e33p get names of "11"&"33" from filter size. They will produce
    # same-sized saliency map because of no-padding for 11-filter, same-padding
    # for 33 filter. (SAME padding is as Half padding, keeps the dimension the
    # same for input and output, while full padding will actually expand the
    # output dimension, as it padds the input to make sure that every element
    # will have the kernel convols once.)
    with tf.variable_scope("fire"):
        ...
        with tf.variable_scope("e33"):
            ...
            e33 = tf.nn.relu(e33)
        # concatenate in the last layer or the channel layer
...
class SqueezeNet(object):
    ...
    x -> conv2d -> relu -> maxpool -> FIRE -> FIRE -> maxpool -> FIRE -> FIRE
    -> maxpool -> FIRE -> FIRE -> conv2d -> avgpool
    """
    ...
    def extract_features(self, input=None, reuse=True):
        ...
        with tf.variable_scope('features', reuse=reuse):
            # (zy) Convolution layer with 64 3*3*3 filters
            # stride size = [1,2,2,1] w/o padding
            # stride size = [batch, h, w, channels]
